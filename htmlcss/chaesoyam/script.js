...
function makeClone() {
  ...
  slides.insertBefore(cloneSlide_second, slides.firstElementChild);
}
function initfunction() {
  slides.style.width = (slideWidth + slideMargin) * (slideCount + 2) + 265 + 'px';
...
next.addEventListener('click', function () {
  ...
  if (currentIdx <= slideCount - 1) {
    ...
    slides.style.transition = `${.3}s ease-out`; //이동 속도
  }
...
prev.addEventListener('click', function () {
  ...
  if (currentIdx >= 0) {
    ...
    slides.style.transition = `${.3}s ease-out`;
  }
  if (currentIdx === 0) {
    setTimeout(function () {
